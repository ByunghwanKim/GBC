...
def show_detail_dialog(row):
    ...
    with col_left:
        ...
        if iv not in ['-', '']: html_vars += f"<span class='badge badge-iv'>IV (독립)</span> <span class='var-text'>{safe(iv)}</span><br>"
        if m not in ['-', '']: html_vars += f"<span class='badge badge-m'>Med (매개)</span> <span class='var-text'>{safe(m)}</span><br>"
        if mod not in ['-', '']: html_vars += f"<span class='badge badge-mod'>Mod (조절)</span> <span class='var-text'>{safe(mod)}</span><br>"
        if dv not in ['-', '']: html_vars += f"<span class='badge badge-dv'>DV (종속)</span> <span class='var-text'>{safe(dv)}</span>"
        html_vars += "</div>"
                ...
                for idx, row in filtered_df.iterrows():
                    with st.container(border=True):
                        ...
                        with c1:
                            ...
                            if iv not in ['-', '']: html_vars += f"<span class='badge badge-iv'>IV (독립)</span><span class='var-text'>{safe(iv)}</span>"
                            if m not in ['-', '']: html_vars += f"<span class='badge badge-m'>Med (매개)</span><span class='var-text'>{safe(m)}</span>"
                            if mod not in ['-', '']: html_vars += f"<span class='badge badge-mod'>Mod (조절)</span><span class='var-text'>{safe(mod)}</span>"
                            if dv not in ['-', '']: html_vars += f"<span class='badge badge-dv'>DV (종속)</span><span class='var-text'>{safe(dv)}</span>"
                            html_vars += "</div>"
